...
const FAQ: { cat: string; icon: string; q: string; a: string }[] = [
  ...
  { cat: "Installation/Uninstallation", icon: "🧹", q: "How do I uninstall the agent (including hidden mode)?", a: "Employees → ⋯ (More) → Uninstaller (Windows or Mac). On Windows run the .bat as administrator; on a Mac double-click the .command, or run it with bash in Terminal if macOS blocks it. Either way it stops and removes the agent and auto-deactivates that employee so the seat frees up while the data is kept. Anyone can also get one from workk.work/uninstall." },
