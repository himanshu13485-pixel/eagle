...
export class EmployeesService {
  ...
  async buildInstaller(orgId: string, employeeId: string, os: "win" | "mac" | "linux" = "win") {
    ...
    if (os === "mac") {
      const sh = [
        ...
        'mkdir -p "$DIR"',
        "",
        // Download to a side file and validate it BEFORE touching the running
        // agent. This used to stop the old agent first and then carry on after a
        // failed download — registering launchd for a file that didn't exist and
        // printing "Done" — so a re-run on a bad network broke a working install.
        'echo "Downloading agent..."',
        `URL="${exeUrl}?os=mac"`,
        // --retry: the shared host drops a small share of new connections.
        'curl -fsSL --retry 3 --retry-delay 2 --connect-timeout 20 -o "$BIN.part" "$URL"',
        "RC=$?",
        'if [ $RC -ne 0 ]; then',
        '  rm -f "$BIN.part"',
        '  echo ""',
        '  if [ $RC -eq 6 ] || [ $RC -eq 7 ] || [ $RC -eq 28 ]; then',
        '    echo "ERROR: this Mac could not reach $SERVER."',
        '    echo "Check it is online (open $SERVER in Safari), then run this installer again."',
        '  elif [ $RC -eq 22 ]; then',
        '    echo "ERROR: the server has no Mac agent to download yet."',
        '    echo "Ask your Workk administrator to publish the macOS build, then run this again."',
        '  else',
        '    echo "ERROR: the download failed (curl error $RC)."',
        '  fi',
        '  echo "Nothing was installed or changed."',
        '  exit 1',
        'fi',
        "",
        // An HTML error page saved as the binary would otherwise "install" fine.
        'if ! file "$BIN.part" | grep -q "Mach-O"; then',
        '  rm -f "$BIN.part"',
        '  echo "ERROR: the download was not a Mac program (the server may have sent an error page)."',
        '  echo "Nothing was installed or changed."',
        '  exit 1',
        'fi',
        // An Apple Silicon build cannot run on an Intel Mac. (The reverse works,
        // through Rosetta, so only the Intel case is fatal.)
        'if [ "$(uname -m)" = "x86_64" ] && ! file "$BIN.part" | grep -q "x86_64"; then',
        '  rm -f "$BIN.part"',
        '  echo "ERROR: this agent was built for Apple Silicon and cannot run on this Intel Mac."',
        '  echo "Ask your Workk administrator for an Intel (x86_64) build."',
        '  exit 1',
        'fi',
        "",
        // Only now replace the running agent.
        ...
        // ~/Library/LaunchAgents does not exist on a fresh macOS account — it is
        // only created when something first installs a launch agent. Writing
        // into it blind failed silently there, so the agent never auto-started.
        ...
        "",
        'if [ ! -s "$PLIST" ]; then',
        '  echo "ERROR: could not write the auto-start file at $PLIST."',
        '  exit 1',
        'fi',
        'if ! launchctl load "$PLIST"; then',
        '  echo "ERROR: macOS refused to register the agent to start automatically."',
        '  echo "The agent is at $BIN — ask your Workk administrator for help."',
        '  exit 1',
        'fi',
        'echo ""',
    ...
    const bat = [
      ...
      "",
      "echo Downloading agent...",
      ":: -f matters: without it curl saves the error body on a 404, the exist",
      ":: check below passes, and Windows tries to run a JSON error as a program.",
      'where curl.exe >nul 2>&1 && curl.exe -fL -o "%EXE%" "%EXE_URL%"',
      ...
      ":: The agent registers its own task: logon + restart-if-stopped every 5 min,",
      ":: allowed on battery. A plain ONLOGON task skips laptops on battery and never",
      ":: restarts a stopped agent. Older agents lack the flag, hence the fallback.",
      '"%EXE%" --register-task >nul 2>&1',
      'if errorlevel 1 schtasks /Create /TN "EagleAgent" /TR "wscript.exe \\"%INSTALL_DIR%\\launch.vbs\\"" /SC ONLOGON /RL HIGHEST /F >nul 2>&1',
      "",
  ...
  async buildUninstaller(orgId: string, employeeId: string, os: "win" | "mac" | "linux" = "win") {
    ...
    const bat = [
      ...
      ":: Remove any website-blocking rules the agent applied (browser policies + hosts).",
      "echo Clearing website blocks...",
      "powershell -NoProfile -Command \"'HKLM:\\SOFTWARE\\Policies\\Google\\Chrome\\URLBlocklist','HKLM:\\SOFTWARE\\Policies\\Microsoft\\Edge\\URLBlocklist','HKLM:\\SOFTWARE\\Policies\\BraveSoftware\\Brave\\URLBlocklist','HKLM:\\SOFTWARE\\Policies\\Mozilla\\Firefox\\WebsiteFilter\\Block' | ForEach-Object { Remove-Item -Path $_ -Recurse -Force -ErrorAction SilentlyContinue }\" >nul 2>&1",
      "powershell -NoProfile -Command \"$h=\\\"$env:WINDIR\\System32\\drivers\\etc\\hosts\\\"; try{ $c=Get-Content -Raw $h; $c=[regex]::Replace($c,'(?ms)\\r?\\n?# EAGLE-BLOCK-START.*?# EAGLE-BLOCK-END',''); Set-Content -Path $h -Value $c.TrimEnd() -Encoding ASCII -Force }catch{}\" >nul 2>&1",
